...
object parser {
  ...
  // Rvalue parsers
  lazy val rvalue: Parsley[RValue]      = call <|> expr <|> pairelem <|> newPair <|> arrayLiter <|> structLiter
  ...
  lazy val waccType: Parsley[Type]    = attempt(arrayType) <|> baseType <|> pairType <|> voidType <|> structType
  ...
  lazy val arrayType: Parsley[ArrayType] = chain.postfix1((baseType <|> structType <|> pairType), (OPENSQUAREBRAC ~> CLOSESQUAREBRAC) #> ArrayType)
